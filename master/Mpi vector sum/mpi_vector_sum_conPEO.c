...
int main(int argc, char* argv[]) {
    ...
    int rank, size;
    MPI_Comm_rank(MPI_COMM_WORLD, &rank); // Communicator per ottenere il rank del processo
    MPI_Comm_size(MPI_COMM_WORLD, &size); // Communicator per ottenere il numero totale di processi
...
//preambolo
    int type = 93; // 23 in hex CODICE OTTENUTO GRAZIE A Perf -v -> Fornito dalla professoressa Loreti
    long config = 0x02; // Codice dell'evento per energy-cores
...
//epilogo
    ...
    long int counter_after; // valore finale del contatore RAPL da leggere
    if (read(fd, &counter_after, sizeof(long int)) != sizeof(long int)) { //legge il contatore; se non legge tutti i byte attesi, è errore
        perror("read");
        close(fd);
        return 1;
    }
    ...
    double scale = 2.3283064365386962890625e-10; // from energy-cores.scale fattore di scala per convertire i valore presente nel counter in unità di misura
    double energy = (counter_after - counter_before) * scale; // Differenza tra i due contatori moltiplicata per il fattore di scala
    // Importante spiegare qui perchè si fa la differenza tra i due contatori e fare qualche cenno riguardo a possibili overflow che qui non gestiamo
    // e soprattutto spiegare che la misurazione avviene ogni 978 microsecondi (valore di default del RAPL) e che quindi se il calcolo dura meno di questo tempo la misurazione non è precisa
    printf("Energy usage: %.100f Joules\n", energy);
