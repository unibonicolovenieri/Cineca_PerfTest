...
#SBATCH --nodes=2                       # Numero di nodi - ULTIMO RUN CON DUE - PRIMO CON 1
...
#perf stat mpiexec -n 2 ./mpi_vector_sum   # visto da stackoverflow con Daniela, qui c'è questa funzione che segue i processi mpi figli SOLO SULLO STESSO NODO
mpiexec -n 2 perf stat ./my-mpi-app
mpiexec -n 2 perf stat ./mpi_vector_sum # Switch to questa se usi piu nodi per seguirli anche nei nodi successivi
...
perf stat -e task-clock,context-switches,cpu-migrations,page-faults,cycles,instructions,branches,branch-misses -p $$ >> $PERF_OUTPUT 
